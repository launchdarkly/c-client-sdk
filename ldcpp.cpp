...
bool LDClient::boolVariationDetail(const std::string &key,
...
int LDClient::intVariationDetail(const std::string &key,
...
double LDClient::doubleVariationDetail(const std::string &key,
...
std::string LDClient::stringVariationDetail(const std::string &key,
    const std::string &def, LDVariationDetails *const details)
{
    char *const s = LDStringVariationAllocDetail(this->client, key.c_str(), def.c_str(), details);
    const std::string res(s); free(s);
    return res;
}
...
char *LDClient::stringVariationDetail(const std::string &key, const std::string &def,
    char *const buf, const size_t len, LDVariationDetails *const details)
{
    return LDStringVariationDetail(this->client, key.c_str(), def.c_str(), buf, len, details);
}

char *LDClient::stringVariation(const std::string &key,
    const std::string &def, char *const buf, const size_t len)
{
    return LDStringVariation(this->client, key.c_str(), def.c_str(), buf, len);
}

LDNode *LDClient::JSONVariationDetail(const std::string &key,
    const LDNode *const def, LDVariationDetails *const details)
{
